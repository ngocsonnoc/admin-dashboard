...
const ThemeMenu = () => {
  ...
  const dispatch = useDispatch()
    ...
    const handleDisableMenu = () => menu_ref.current.classList.remove('active')
    useEffect(() => {
        const themeClass = mode_settings.find(e => e.class === localStorage.getItem('themeMode',
            'theme-mode-light'))
        const colorClass = mode_settings.find(e => e.class === localStorage.getItem('colorMode',
            'theme-mode-light'))
    ...
    return (
        <div>
            <button ref={menu_toggle_ref} className='dropdown__toggle' onClick={() => handleActiveMenu()}>
                <i className='bx bx-palette'></i>
            </button>
            <div ref={menu_ref} className='theme-menu'>
                <h4> Theme settings</h4>
                <button className='theme-menu__close' onClick={() => handleDisableMenu()}>
